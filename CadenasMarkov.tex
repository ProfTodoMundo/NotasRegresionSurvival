...
%_____________________________________________________________________________________
%}
%%\begin{frame}\frametitle{Estacionareidad}
...
\begin{Def}
La medida $v$ es estacionaria si $v_{i}<\infty$ para toda $i$ y adem?s $v\prob=v$.\smallskip
\end{Def}
En el caso de que $v$ sea distribuci?n, independientemente de que sea estacionaria o no, se cumple con
...
%%%\begin{frame}\frametitle{Estacionareidad}
\begin{Teo}
Supongamos que $v$ es una distribuci?n estacionaria. Entonces
\begin{itemize}
\item[i)] La cadena es estrictamente estacionaria con respecto a $\prob_{v}$, es decir, $\prob_{v}$-distribuci?n de $\left\{X_{n},X_{n+1},\ldots\right\}$ no depende de $n$;
\item[ii)] Existe un aversi?n estrictamente estacionaria $\left\{X_{n}\right\}_{n\in Z}$ de la cadena con doble tiempo infinito y $\prob\left(X_{n}=i\right)=v_{i}$ para toda $n\in Z$.
\end{itemize}
\end{Teo}
\begin{Teo}
Sea $i$ estado fijo, recurrente. Entonces una medida estacionaria $v$ puede definirse haciendo que $v_{j}$ sea el n?mero esperado de visitas a $j$ entre dos visitas consecutivas $i$,
...
Si la cadena es irreducible y recurrente, entonces una medida estacionaria $v$ existe, satisface $0<v_{j}<\infty$ para toda $j$ y es ?nica salvo factores multiplicativos, es decir, si $v,v^{*}$ son estacionarias, entonces $c=cv^{*}$ para alguna $c\in\left(0,\infty\right)$.
\end{Teo}
\begin{Cor}\label{Cor.3.5}
Si la cadena es irreducible y positiva recurrente, existe una ?nica distribuci?n estacionaria $\pi$ dada por
\begin{equation}
...
%_____________________________________________________________________________________
%
\subsection{Teor?a Erg?dica}
%_____________________________________________________________________________________
%
%%%\begin{frame}\frametitle{Estacionareidad y Teor?a Erg?dica}
\begin{Lema}
...
\begin{Teo}
Sea $\left\{X_{n}\right\}$ cadena irreducible y aperi?dica positiva recurrente, y sea $\pi=\left\{\pi_{j}\right\}_{j\in E}$ la distribuci?n estacionaria. Entonces $p_{ij}^{n}\rightarrow\pi_{j}$ para todo $i,j$.
\end{Teo}
\begin{Def}\label{Def.Ergodicidad}
Una cadena irreducible aperiodica, positiva recurrente con medida estacionaria $v$, es llamada {\em erg?dica}.
\end{Def}
%\end{frame}
%____________________________________________________________________________
%%%\begin{frame}\frametitle{Estacionareidad}
...
La matriz $\widetilde{P}$ con elementos $\widetilde{p}_{ij}=\frac{v_{ji}p_{ji}}{v_{i}}$ es una matriz de transici?n. Adem?s, el $i$-?simo elementos $\widetilde{p}_{ij}^{m}$ de la matriz potencia $\widetilde{P}^{m}$ est? dada por $\widetilde{p}_{ij}^{m}=\frac{v_{ji}p_{ji}^{m}}{v_{i}}$.
\end{Lema}

\begin{Lema}
Def?nase $N_{i}^{m}=\sum_{n=0}^{m}\indora\left(X_{n}=i\right)$ como el n?mero de visitas a $i$ antes del tiempo $m$. Entonces si la cadena es reducible y recurrente, $lim_{m\rightarrow\infty}\frac{\esp_{j}N_{i}^{m}}{\esp_{k}N_{i}^{m}}=1$ para todo $j,k\in E$.
\end{Lema}
%\end{frame}
%_____________________________________________________________________________________
%
\subsection{Funciones Arm?nicas, Recurrencia y Transitoriedad}
%_____________________________________________________________________________________
%
%%%\begin{frame}\frametitle{Criteros de Recurrencia y Transitoriedad}
\begin{Def}\label{Def.Armonica}
Una funci?n Arm?nica es el eigenvector derecho $h$ de $P$ corrrespondiente al eigenvalor 1.
\end{Def}
\begin{eqnarray*}
...
Sea $\left\{X_{n}\right\}$ cadena irreducible  y sea $i$ estado fijo arbitrario. Entonces la cadena es transitoria s? y s?lo si existe una funci?n no cero, acotada $h:E-\left\{i\right\}\rightarrow\rea$ que satisface
...
%\end{frame}
%____________________________________________________________________________
%%%\begin{frame}\frametitle{Funciones Arm?nicas}
...
Suponga que la cadena es irreducible y sea $E_{0}$ un subconjunto finito de $E$ tal que se cumple la ecuaci?n 5.2 para alguna funci?n $h$ acotada que satisface $h\left(i\right)<h\left(j\right)$ para alg?n estado $i\notin E_{0}$ y todo $j\in E_{0}$. Entonces la cadena es transitoria.
\end{Prop}
%\end{frame}
...
%_____________________________________________________________________________________
%
\subsection{Estructura B?sica de los Procesos Markovianos de Saltos}
%_____________________________________________________________________________________
%
%%%\begin{frame}\frametitle{Estructura B?sica de los Procesos Markovianos de Saltos}
\begin{itemize}
...
\item El monto del tiempo gastado en cada estado es positivo, de modo tal que las trayectorias muestrales son constantes por partes. Para un proceso de saltos denotamos por los tiempos de saltos a $S_{0}=0<S_{1}<S_{2}\cdots$, los tiempos entre saltos consecutivos $T_{n}=S_{n+1}-S_{n}$ y la secuencia de estados visitados por $Y_{0},Y_{1},\ldots$, as? las trayectorias muestrales son constantes entre $S_{n}$ consecutivos, continua por la derecha, es decir, $X_{S_{n}}=Y_{n}$.
\item La descripci?n de un modelo pr?ctico est? dado usualmente en t?rminos de las intensidades $\lambda\left(i\right)$ y las probabilidades de salto $q_{ij}$ m?s que en t?rminos de la matriz de transici?n $P^{t}$.
\item Sup?ngase de ahora en adelante que $q_{ii}=0$ cuando $\lambda\left(i\right)>0$
\end{itemize}
%\end{frame}
%_____________________________________________________________________________________
%
...
%%%\begin{frame}\frametitle{Matriz Intensidad}
\begin{Def}
La matriz intensidad $\Lambda=\left(\lambda\left(i,j\right)\right)_{i,j\in E}$ del proceso de saltos $\left\{X_{t}\right\}_{t\geq0}$ est? dada por
\begin{eqnarray*}
...
Una matriz $E\times E$, $\Lambda$ es la matriz de intensidad de un proceso markoviano de saltos $\left\{X_{t}\right\}_{t\geq0}$ si y s?lo si
\begin{eqnarray*}
\lambda\left(i,i\right)\leq0\textrm{,  }\lambda\left(i,j\right)\textrm{,   }i\neq j\textrm{,  }\sum_{j\in E}\lambda\left(i,j\right)=0.
\end{eqnarray*}
Adem?s, $\Lambda$ est? en correspondencia uno a uno con la distribuci?n del proceso minimal dado por el teorema 3.1.
\end{Prop}
...
%%%\begin{frame}\frametitle{Matriz Intensidad}
Para el caso particular de la Cola $M/M/1$, la matr?z de itensidad est? dada por
\begin{eqnarray*}
...
%____________________________________________________________________________
%
%%%\begin{frame}\frametitle{Medidas Estacionarias}
...
Supongamos que $\left\{X_{t}\right\}$ es irreducible recurrente en $E$. Entonces existe una y s?lo una, salvo m?ltiplos, medida estacionaria $v$. Esta $v$ tiene la propiedad de que $0< v_{j}<\infty$ para todo $j$ y puede encontrarse en cualquiera de las siguientes formas

\begin{itemize}
\item[i)] Para alg?n estado $i$, fijo pero arbitrario, $v_{j}$ es el tiempo esperado utilizado en $j$ entre dos llegadas consecutivas al estado $i$;
...
\item[iii)] como soluci?n de $v\Lambda=0$.
\end{itemize}
\end{Teo}
%\end{frame}
%____________________________________________________________________________
...
%____________________________________________________________________________
%
%%%\begin{frame}\frametitle{Criterios de Ergodicidad}
\begin{Def}
Un proceso irreducible recurrente con medida estacionaria de masa finita es llamado erg?dico.
\end{Def}

\begin{Teo}\label{Teo.4.3}
Un proceso de Markov de saltos irreducible no explosivo es erg?dico si y s?lo si se puede encontrar una soluci?n, de probabilidad, $\pi$, con $|\pi|=1$ y $0\leq\pi_{j}\leq1$, a $\pi\Lambda=0$. En este caso $\pi$ es la distribuci?n estacionaria.
\end{Teo}

\begin{Cor}\label{Cor.4.4}
Una condici?n suficiente para la ergodicidad de un proceso irreducible es la existencia de una probabilidad $\pi$ que resuelva el sistema $\pi\Lambda=0$ y que adem?s tenga la propiedad de que $\sum\pi_{j}\lambda\left(j\right)<\infty$.
\end{Cor}
%\end{frame}
%____________________________________________________________________________
%%%\begin{frame}\frametitle{Criterios de Ergodicidad}
\begin{Prop}
Si el proceso es erg?dico, entonces existe una versi?n estrictamente estacionaria $\left\{X_{t}\right\}_{-\infty<t<\infty}$con doble tiempo infinito.
\end{Prop}

\begin{Teo}
Si $\left\{X_{t}\right\}$ es erg?dico y $\pi$ es la distribuci?n estacionaria, entonces para todo $i,j$, $p_{ij}^{t}\rightarrow\pi_{j}$ cuando $t\rightarrow\infty$.
\end{Teo}

\begin{Cor}
Si $\left\{X_{t}\right\}$ es irreducible recurente pero no erg?dica, es decir $|v|=\infty$, entonces $p_{ij}^{t}\rightarrow0$ para todo $i,j\in E$.
\end{Cor}

\begin{Cor}
Para cualquier proceso Markoviano de Saltos minimal, irreducible o no, los l?mites $li_{t\rightarrow\infty}p_{ij}^{t}$ existe.
\end{Cor}
%\end{frame}
%_____________________________________________________________________________________
%
\section{Notaci?n Kendall-Lee}
%_____________________________________________________________________________________
%

%%%\begin{frame}\frametitle{Notaci?n de Kendall}
A partir de este momento se har?n las siguientes consideraciones: Si $t_{n}$ es el tiempo aleatorio en el que llega al sistema el $n$-?simo cliente, para $n=1,2,\ldots$, $t_{0}=0$ y $t_{0}<t_{1}<\cdots$ se definen los tiempos entre arribos  $\tau_{n}=t_{n}-t_{n-1}$ para $n=1,2,\ldots$, variables aleatorias independientes e id?nticamente distribuidas. Los tiempos entre arribos tienen un valor medio $E\left(\tau\right)$ finito y positivo $\frac{1}{\beta}$, es decir, $\beta$ se puede ver como la tasa o intensidad promedio de arribos al sistema por unidad de tiempo. Adem?s se supondr? que los servidores son identicos y si $s$ denota la variable aleatoria que describe el tiempo de servicio, entonces $E\left(s\right)=\frac{1}{\delta}$, $\delta$ es la tasa promedio de servicio por servidor.

La notaci?n de Kendall-Lee es una forma abreviada de describir un sisema de espera con las siguientes componentes:
\begin{itemize}
\item[i)] {\em\bf Fuente}: Poblaci?n de clientes potenciales del sistema, esta puede ser finita o infinita.
\item[ii)] {\em\bf Proceso de Arribos}: Proceso determinado por la funci?n de distribuci?n $A\left(t\right)=P\left\{\tau\leq t\right\}$ de los tiempos entre arribos.
\end{itemize}
%\end{frame}
...
Adem?s tenemos las siguientes igualdades
\begin{equation}\label{Eq.0.1}
N\left(t\right)=N_{q}\left(t\right)+N_{s}\left(s\right)
\end{equation}
donde
\begin{itemize}
\item $N\left(t\right)$ es el n?mero de clientes en el sistema al tiempo $t$.
\item $N_{q}\left(t\right)$ es el n?mero de clientes en la cola al tiempo $t$
\item $N_{s}\left(t\right)$ es el n?mero de clientes recibiendo servicio en el tiempo $t$.
\end{itemize}

Bajo la hip?tesis de estacionareidad, es decir, las caracter?sticas de funcionamiento del sistema se han estabilizado en valores independientes del tiempo, entonces
\begin{equation}
N=N_{q}+N_{s}.
\end{equation}

Los valores medios de las cantidades anteriores se escriben como $L=E\left(N\right)$, $L_{q}=E\left(N_{q}\right)$ y $L_{s}=E\left(N_{s}\right)$, entonces de la ecuaci?n \ref{Eq.0.1} se obtiene
...
La intensidad de tr?fico se define como
\begin{equation}
\rho=\frac{E\left(s\right)}{E\left(\tau\right)}=\frac{\beta}{\delta}.
\end{equation}

La utilizaci?n por servidor es
\begin{equation}
u=\frac{\rho}{c}=\frac{\beta}{c\delta}.
\end{equation}
donde $c$ es el n?mero de servidores.
%\end{frame}
%____________________________________________________________________________
%%%\begin{frame}\frametitle{M?s sobre la notaci?n {\em Kendall-Lee}}
Esta notaci?n es una forma abreviada de describir un sistema de espera con componentes dados a continuaci?n, la notaci?n es
\begin{equation}\label{Notacion.K.L.}
A/S/c/K/F/d
\end{equation}
Cada una de las letras describe:
\begin{itemize}
\item $A$ es la distribuci?n de los tiempos entre arribos.
\item $S$ es la distribuci?n del tiempo de servicio.
\item $c$ es el n?mero de servidores.
\item $K$ es la capacidad del sistema.
\item $F$ es el n?mero de individuos en la fuente.
...
\begin{itemize}
\item $GI$ para la distribuci?n general de los tiempos entre arribos.
\item $G$ distribuci?n general del tiempo de servicio.
\item $M$ Distribuci?n exponencial para $A$ o $S$.
\item $E_{K}$ Distribuci?n Erlang-$K$, para $A$ o $S$.
...
Por un proceso de nacimiento y muerte se entiende un proceso de saltos de markov $\left\{X_{t}\right\}_{t\geq0}$ con espacio de estados a lo m?s numerable, con la propiedad de que s?lo puede ir al estado $n+1$ o al estado $n-1$, es decir, su matriz de intensidad es de la forma
\begin{eqnarray*}
\Lambda=\left[\begin{array}{cccccc}
...
\end{array}\right]
\end{eqnarray*}
donde $\beta_{n}$ son las intensidades de nacimiento y $\delta_{n}$ las intensidades de muerte, o tambi?n se puede ver como a $X_{t}$ el n?mero de usuarios en una cola al tiempo $t$, un salto hacia arriba corresponde a la llegada de un nuevo usuario y un salto hacia abajo como al abandono de un usuario despu?s de haber recibido su servicio.

La cadena de saltos $\left\{Y_{n}\right\}$ tiene matriz de transici?n dada por
\begin{eqnarray*}
...
\end{array}\right]
\end{eqnarray*}
donde $p_{n}=\frac{\beta_{n}}{\beta_{n}+\delta_{n}}$ y $q_{n}=1-p_{n}=\frac{\delta_{n}}{\beta_{n}+\delta_{n}}$, donde adem?s se asumne por el momento que $p_{n}$ no puede tomar el valor $0$ ? $1$ para cualquier valor de $n$.
%\end{frame}
%____________________________________________________________________________
...
Independientemente de la recurrencia o transitorieadad, existe una y s?lo una, salvo m?ltiplos, soluci?n a $v\Lambda=0$, dada por
...
En el caso recurrente, la medida estacionaria $\mu$ para $\left\{Y_{n}\right\}$ est? dada por
...
$\left\{X_{t}\right\}$ es erg?dica si y s?lo si la ecuaci?n (\ref{Eq.2.1}) se cumple y adem?s $S<\infty$, en cuyo caso la distribuci?n erg?dica, $\pi$, est? dada por
...
%%%\begin{frame}\frametitle{Cola $M/M/1$}
Este modelo corresponde a un proceso de nacimiento y muerte con $\beta_{n}=\beta$ y $\delta_{n}=\delta$ independiente del valor de $n$. La intensidad de tr?fico $\rho=\frac{\beta}{\delta}$, implica que el criterio de recurrencia (ecuaci?n \ref{Eq.2.1}) quede de la forma:
\begin{eqnarray*}
1+\sum_{n=1}^{\infty}\rho^{-n}=\infty.
\end{eqnarray*}
Equivalentemente el proceso es recurrente si y s?lo si
\begin{eqnarray*}
\sum_{n\geq1}\left(\frac{\beta}{\delta}\right)^{n}<\infty\Leftrightarrow \frac{\beta}{\delta}<1
\end{eqnarray*}
Entonces
$S=\frac{\delta}{\delta-\beta}$, luego por la ecuaci?n \ref{Eq.2.4} se tiene que
\begin{eqnarray*}
...
\begin{Prop}
La cola $M/M/1$ con intendisad de tr?fico $\rho$, es recurrente si y s?lo si $\rho\leq1$.
\end{Prop}

Entonces por el corolario \ref{Cor.2.3}

\begin{Prop}
La cola $M/M/1$ con intensidad de tr?fico $\rho$ es erg?dica si y s?lo si $\rho<1$. En cuyo caso, la distribuci?n de equilibrio $\pi$ de la longitud de la cola es geom?trica, $\pi_{n}=\left(1-\rho\right)\rho^{n}$, para $n=1,2,\ldots$.
\end{Prop}
De la proposici?n anterior se desprenden varios hechos importantes.
\begin{enumerate}
\item $\prob\left[X_{t}=0\right]=\pi_{0}=1-\rho$, es decir, la probabilidad de que el sistema se encuentre ocupado.
\item De las propiedades de la distribuci?n Geom?trica se desprende que
\begin{enumerate}
...
Si $L$ es el n?mero esperado de clientes en el sistema, incluyendo los que est?n siendo atendidos, entonces
\begin{eqnarray*}
L=\frac{\rho}{1-\rho}
\end{eqnarray*}
Si adem?s $W$ es el tiempo total del cliente en la cola: $W=W_{q}+W_{s}$
$\rho=\frac{\esp\left[s\right]}{\esp\left[\tau\right]}=\beta W_{s}$, puesto que $W_{s}=\esp\left[s\right]$ y $\esp\left[\tau\right]=\frac{1}{\delta}$. Por la f?rmula de Little $L=\lambda W$
\begin{eqnarray*}
...
%_____________________________________________________________________________________
%
%%%\begin{frame}\frametitle{Cola $M/M/\infty$}
Este tipo de modelos se utilizan para estimar el n?mero de l?neas en uso en una gran red comunicaci?n o para estimar valores en los sistemas $M/M/c$ o $M/M/c/c$, en el se puede pensar que siempre hay un servidor disponible para cada cliente que llega.\smallskip

Se puede considerar como un proceso de nacimiento y muerte con par?metros $\beta_{n}=\beta$ y $\mu_{n}=n\mu$ para $n=0,1,2,\ldots$, entonces por la ecuaci?n \ref{Eq.2.4} se tiene que
\begin{eqnarray*}\label{MMinf.pi}
\pi_{0}=e^{\rho}\\
\pi_{n}=e^{-\rho}\frac{\rho^{n}}{n!}
\end{eqnarray*}
Entonces, el n?mero promedio de servidores ocupados es equivalente a considerar el n?mero de clientes en el  sistema, es decir,
\begin{eqnarray*}
...
%____________________________________________________________________________
%%%\begin{frame}\frametitle{Cola $M/M/\infty$}
Adem?s se tiene que $W_{q}=0$ y $L_{q}=0$.\smallskip

El tiempo promedio en el sistema es el tiempo promedio de servicio, es decir, $W=\esp\left[s\right]=\frac{1}{\delta}$.
Resumiendo, tenemos la sisuguiente proposici?n:
\begin{Prop}
La cola $M/M/\infty$ es erg?dica para todos los valores de $\eta$. La distribuci?n de equilibrio $\pi$ es Poisson con media $\eta$, $\pi_{n}=\frac{e^{-n}\eta^{n}}{n!}$.
\end{Prop}
%\end{frame}
%_____________________________________________________________________________________
%
...
%_____________________________________________________________________________________
%
%%%\begin{frame}\frametitle{Cola $M/M/m$}
Este sistema considera $m$ servidores id?nticos, con tiempos entre arribos y de servicio exponenciales con medias $\esp\left[\tau\right]=\frac{1}{\beta}$ y $\esp\left[s\right]=\frac{1}{\delta}$. definimos ahora la utilizaci?n por servidor como $u=\frac{\rho}{m}$ que tambi?n se puede interpretar como la fracci?n de tiempo promedio que cada servidor est? ocupado.\smallskip

La cola $M/M/m$ se puede considerar como un proceso de nacimiento y muerte con par?metros: $\beta_{n}=\beta$ para $n=0,1,2,\ldots$ y $\delta_{n}=\left\{\begin{array}{cc}
n\delta & n=0,1,\ldots,m-1\\
c\delta & n=m,\ldots\\
\end{array}\right.$

entonces  la condici?n de recurrencia se va a cumplir s? y s?lo si $\sum_{n\geq1}\frac{\beta_{0}\cdots\beta_{n-1}}{\delta_{1}\cdots\delta_{n}}<\infty$, equivalentemente se debe de cumplir que
...
W=W_{q}+\frac{1}{\delta}
\end{equation}
Si definimos $C\left(m,\rho\right)=\frac{\pi_{0}\rho^{m}}{m!\left(1-u\right)}=\frac{\pi_{m}}{1-u}$, que es la probabilidad de que un cliente que llegue al sistema tenga que esperar en la cola. Entonces podemos reescribir las ecuaciones reci?n enunciadas:
...
%%%\begin{frame}\frametitle{Cola $M/M/m$}
\begin{Prop}
La cola $M/M/m$ con intensidad de tr?fico $\rho$ es erg?dica si y s?lo si $\rho<1$. En este caso la distribuci?n erg?dica $\pi$ est? dada por
\begin{eqnarray*}
\pi_{n}=\left\{\begin{array}{cc}
...
%_____________________________________________________________________________________
%
%%%\begin{frame}\frametitle{Cola M/G/1}
Consideremos un sistema de espera con un servidor, en el que los tiempos entre arribos son exponenciales, y los tiempos de servicio tienen una distribuci?n general $G$. Sea $N\left(t\right)_{t\geq0}$ el n?mero de clientes en el sistema al tiempo $t$, y sean $t_{1}<t_{2}<\dots$ los tiempos sucesivos en los que los clientes completan su servicio y salen del sistema.

La sucesi?n $\left\{X_{n}\right\}$ definida por $X_{n}=N\left(t_{n}\right)$ es una cadena de Markov, en espec?fico es la Cadena encajada del proceso de llegadas de usuarios. Sea $U_{n}$ el n?mero de clientes que llegan al sistema durante el tiempo de servicio del $n$-?simo cliente, entonces se tiene que
...
%____________________________________________________________________________
%%%\begin{frame}\frametitle{Cola M/G/1}
Dado que los procesos de arribos de los usuarios es Poisson con par?metro $\lambda$, la probabilidad condicional de que lleguen $j$ clientes al sistema dado que el tiempo de servicio es $s=t$, resulta:
\begin{eqnarray*}
...
\end{equation}
donde $G$ es la distribuci?n de los tiempos de servicio. Las probabilidades de transici?n de la cadena est?n dadas por
\begin{equation}
...
\right.
\end{equation}
Entonces la matriz de transici?n es:
\begin{eqnarray*}
...
%____________________________________________________________________________
%%%\begin{frame}\frametitle{Cola M/G/1}
Recordemos que si la cadena de Markov $\left\{X_{n}\right\}$ tiene una distribuci?n estacionaria entonces existe una distribuci?n de probabilidad $\pi=\left(\pi_{0},\pi_{1},\ldots,\right)$, con $\pi_{i}\geq0$ y $\sum_{i\geq1}\pi_{i}=1$ tal que satisface la ecuaci?n $\pi=\pi P$, equivalentemente
...
%____________________________________________________________________________
%%%\begin{frame}\frametitle{Cola M/G/1}
Si la ecuaci?n \ref{Eq.19.6} la multiplicamos por $z^{j}$ y sumando sobre $j$, se tiene que
\begin{eqnarray*}
...
%____________________________________________________________________________
%%%\begin{frame}\frametitle{Cola M/G/1}
Si $z\rightarrow1$, entonces $A\left(z\right)\rightarrow A\left(1\right)=1$, y adem?s $A^{'}\left(z\right)\rightarrow A^{'}\left(1\right)=\rho$. Si aplicamos la Regla de L'Hospital se tiene que
\begin{eqnarray*}
...
\end{eqnarray*}
%\end{frame}
%begin{frame}\frametitle{Cola M/G/1}%\end{frame}
%____________________________________________________________________________
%%%\begin{frame}\frametitle{Cola M/G/1}
Adem?s, se tiene que $\rho=\beta\esp\left[s\right]=\frac{\beta}{\delta}$ y la distribuci?n estacionaria est? dada por
\begin{eqnarray}
\pi_{j}&=&\pi_{0}a_{j}+\sum_{i=1}^{j+1}\pi_{i}a_{j-i+1}\textrm{, para }j=0,1,\ldots\\
\pi_{0}&=&1-\rho
\end{eqnarray}
...
De las f?rmulas de Little, se tiene que $W=E\left(w\right)=\frac{L}{\beta}$, tambi?n el tiempo de espera en la cola
\begin{equation}
W_{q}=\esp\left(q\right)=\esp\left(w\right)-\esp\left(s\right)=\frac{L}{\beta}-\frac{1}{\delta},
\end{equation}
adem?s el n?mero promedio de clientes en la cola es
\begin{equation}
...
adem?s $v\prob=v$.\smallskip
\end{Def}
En el caso de que $v$ sea distribuci?n, independientemente de que
...
$\prob_{v}$, es decir, $\prob_{v}$-distribuci?n de
$\left\{X_{n},X_{n+1},\ldots\right\}$ no depende de $n$;
\item[ii)] Existe una versi?n estrictamente estacionaria
...
$v$ puede definirse haciendo que $v_{j}$ sea el n?mero esperado de
...
?nica distribuci?n estacionaria $\pi$ dada por
\begin{equation}
\pi_{j}=\frac{1}{\esp_{i}\tau_{i}}\esp_{i}\sum_{n=0}^{\tau\left(i\right)-1}\indora\left(X_{n}=j\right)=\frac{1}{\esp_{j}\tau\left(j\right)}.
\end{equation}
\end{Cor}
...
\begin{Teo}
Sea $\left\{X_{n}\right\}$ cadena irreducible y aperi?dica
...
%%%\begin{frame}\frametitle{Estructura B?sica}
...
$Y_{0},Y_{1},\ldots$, as? las trayectorias muestrales son
...
%\begin{Teo}Consid?rese un proceso de Saltos de Markov, entonces la distribuci?n conjunta de la sucesi?n $\left\{Y_{n}\right\}_{n\in\nat}$ de los estados visitados, antes de explotar, y $\left\{T_{n}\right\}_{n\in\nat}$\end{Teo}
...
finita es llamado erg?dico.
\end{Def}

\begin{Teo}\label{Teo.4.3}
Un proceso de Markov de saltos irreducible no explosivo es
erg?dico si y s?lo si se puede encontrar una soluci?n, de
probabilidad, $\pi$, con $|\pi|=1$ y $0\leq\pi_{j}\leq1$, a
$\pi\Lambda=0$. En este caso $\pi$ es la distribuci?n
estacionaria.
\end{Teo}

\begin{Cor}\label{Cor.4.4}
Una condici?n suficiente para la ergodicidad de un proceso
irreducible es la existencia de una probabilidad $\pi$ que
resuelva el sistema $\pi\Lambda=0$ y que adem?s tenga la propiedad
...
tiempo. Adem?s se supondr? que los servidores son identicos y si
...
La notaci?n de Kendall-Lee es una forma abreviada de describir un
sisema de espera con las siguientes componentes:
\begin{itemize}
\item[i)] {\em\bf Fuente}: Poblaci?n de clientes potenciales del
sistema, esta puede ser finita o infinita. \item[ii)] {\em\bf
Proceso de Arribos}: Proceso determinado por la funci?n de
distribuci?n $A\left(t\right)=P\left\{\tau\leq t\right\}$ de los
...
donde
\begin{itemize}
\item $N\left(t\right)$ es el n?mero de clientes en el sistema al
tiempo $t$. \item $N_{q}\left(t\right)$ es el n?mero de clientes
en la cola al tiempo $t$ \item $N_{s}\left(t\right)$ es el n?mero
de clientes recibiendo servicio en el tiempo $t$.
\end{itemize}

Bajo la hip?tesis de estacionareidad, es decir, las
caracter?sticas de funcionamiento del sistema se han estabilizado
...
$L_{s}=E\left(N_{s}\right)$, entonces de la ecuaci?n \ref{Eq.0.1}
se obtiene
...
%_____________________________________________________________________________________
%
\section{Procesos de Nacimiento y Muerte (Teor?a)}
%_____________________________________________________________________________________
%
%%%\begin{frame}\frametitle{Procesos de Nacimiento y Muerte}
\begin{Prop}\label{Prop.2.1}
...
y s?lo una, salvo m?ltiplos, soluci?n a $v\Lambda=0$, dada por
\begin{equation}\label{Eq.2.2}
v_{n}=\frac{\beta_{0}\cdots\beta_{n-1}}{\delta_{1}\cdots\delta_{n}}v_{0}
\end{equation}
para $n=1,2,\ldots$.
...
\begin{Cor}\label{Cor.2.4}
$\left\{X_{t}\right\}$ es erg?dica si y s?lo si la ecuaci?n
(\ref{Eq.2.1}) se cumple y adem?s $S<\infty$, en cuyo caso la
distribuci?n erg?dica, $\pi$, est? dada por
\begin{equation}\label{Eq.2.4}
...
$n$. La intensidad de tr?fico $\rho=\frac{\beta}{\delta}$, implica
que el criterio de recurrencia (ecuaci?n \ref{Eq.2.1}) quede de la
forma:
\begin{eqnarray*}
1+\sum_{n=1}^{\infty}\rho^{-n}=\infty.
\end{eqnarray*}
...
\frac{\beta}{\delta}<1
\end{eqnarray*}
Entonces $S=\frac{\delta}{\delta-\beta}$, luego por la ecuaci?n
...
\begin{Prop}
La cola $M/M/1$ con intendisad de tr?fico $\rho$, es recurrente si
y s?lo si $\rho\leq1$.
\end{Prop}
...
\begin{Prop}
La cola $M/M/1$ con intensidad de tr?fico $\rho$ es erg?dica si y
s?lo si $\rho<1$. En cuyo caso, la distribuci?n de equilibrio
$\pi$ de la longitud de la cola es geom?trica,
$\pi_{n}=\left(1-\rho\right)\rho^{n}$, para $n=1,2,\ldots$.
\end{Prop}
De la proposici?n anterior se desprenden varios hechos
importantes.
\begin{enumerate}
\item $\prob\left[X_{t}=0\right]=\pi_{0}=1-\rho$, es decir, la
probabilidad de que el sistema se encuentre ocupado. \item De las
propiedades de la distribuci?n Geom?trica se desprende que
\begin{enumerate}
...
Si $L$ es el n?mero esperado de clientes en el sistema, incluyendo
los que est?n siendo atendidos, entonces
\begin{eqnarray*}
L=\frac{\rho}{1-\rho}
\end{eqnarray*}
Si adem?s $W$ es el tiempo total del cliente en la cola:
$W=W_{q}+W_{s}$
$\rho=\frac{\esp\left[s\right]}{\esp\left[\tau\right]}=\beta
W_{s}$, puesto que $W_{s}=\esp\left[s\right]$ y
$\esp\left[\tau\right]=\frac{1}{\delta}$. Por la f?rmula de Little
$L=\lambda W$
\begin{eqnarray*}
W&=&\frac{L}{\beta}=\frac{\frac{\rho}{1-\rho}}{\beta}=\frac{\rho}{\delta}\frac{1}{1-\rho}=\frac{W_{s}}{1-\rho}\\
&=&\frac{1}{\delta\left(1-\rho\right)}=\frac{1}{\delta-\beta}
...
%_____________________________________________________________________________________
%
\section{Notaci?n Kendall-Lee, segunda parte}
%_____________________________________________________________________________________
%
%%%\begin{frame}\frametitle{M?s sobre la notaci?n {\em Kendall-Lee}}
Esta notaci?n es una forma abreviada de describir un sistema de
espera con componentes dados a continuaci?n, la notaci?n es
\begin{equation}\label{Notacion.K.L.}
A/S/c/K/F/d
\end{equation}
Cada una de las letras describe:
\begin{itemize}
\item $A$ es la distribuci?n de los tiempos entre arribos. \item
$S$ es la distribuci?n del tiempo de servicio. \item $c$ es el
n?mero de servidores. \item $K$ es la capacidad del sistema. \item
$F$ es el n?mero de individuos en la fuente. \item $d$ es la
...
Las distribuciones usuales para $A$ y $B$ son:
\begin{itemize}
\item $GI$ para la distribuci?n general de los tiempos entre
arribos. \item $G$ distribuci?n general del tiempo de servicio.
\item $M$ Distribuci?n exponencial para $A$ o $S$. \item $E_{K}$
Distribuci?n Erlang-$K$, para $A$ o $S$. \item $D$ tiempos entre
arribos o de servicio constantes, es decir, deterministicos.
\end{itemize}
%\end{frame}
%_____________________________________________________________________________________
%
\section{Procesos de Nacimiento y Muerte como Modelos de Colas(Continuaci?n)}
%_____________________________________________________________________________________
%
...
$\delta_{n}=n\delta$, en este caso el par?metro de inter?s
$\eta=\frac{\beta}{\delta}$, luego, la ecuaci?n \ref{Eq.2.1} queda
de la forma:
...
\begin{Prop}
La cola $M/M/\infty$ es erg?dica para todos los valores de $\eta$.
La distribuci?n de equilibrio $\pi$ es Poisson con media $\eta$,
...
n?mero de servidores ocupados en el estado $n$, es decir,
$m\left(n\right)=m$, para $n\geq m$ y $m\left(n\right)=m$ para
$1\leq n\leq m$. La intensidad de tr?fico es
$\rho=\frac{\beta}{m\delta}$ y $\frac{\beta_{n}}{\delta_{n}}=\rho$
para $n\geq m$. As?, al igual que en el caso $m=1$, la ecuaci?n
\ref{Eq.2.1} y la recurrencia se cumplen si y s?lo si
...
\end{eqnarray*}
es finita si y s?lo si $\rho<1$, por tanto se tiene la siguiente

\begin{Prop}
La cola $M/M/m$ con intensidad de tr?fico $\rho$ es erg?dica si y
s?lo si $\rho<1$. En este caso la distribuci?n erg?dica $\pi$ est?
dada por
\begin{eqnarray*}
\pi_{n}=\left\{\begin{array}{cc}
\frac{1}{S}\frac{\eta^{n}}{n!} & 0\leq n\leq m\\
...
$\prob_{v}$, es decir, $\prob_{v}$-distribuci?n de
$\left\{X_{n},X_{n+1},\ldots\right\}$ no depende de $n$;
\item[ii)] Existe un aversi?n estrictamente estacionaria
$\left\{X_{n}\right\}_{n\in Z}$ de la cadena con doble tiempo
infinito y $\prob\left(X_{n}=i\right)=v_{i}$ para toda $n\in Z$.
\end{itemize}
\end{Teo}
...
transici?n. Adem?s, el $i$-?simo elementos
...
\begin{Lema}
Def?nase $N_{i}^{m}=\sum_{n=0}^{m}\indora\left(X_{n}=i\right)$
como el n?mero de visitas a $i$ antes del tiempo $m$. Entonces si
...
%%%\begin{frame}\frametitle{Criteros de Recurrencia y Transitoriedad}
\begin{Def}\label{Def.Armonica}
Una funci?n Arm?nica es el eigenvector derecho $h$ de $P$
...
fijo arbitrario. Entonces la cadena es transitoria s? y s?lo si
existe una funci?n no cero, acotada
...
%
%%%\begin{frame}\frametitle{Cola $M/M/\infty$}
Este tipo de modelos se utilizan para estimar el n?mero de l?neas
en uso en una gran red comunicaci?n o para estimar valores en los
...
par?metros $\beta_{n}=\beta$ y $\mu_{n}=n\mu$ para
$n=0,1,2,\ldots$, entonces por la ecuaci?n \ref{Eq.2.4} se tiene
que
\begin{eqnarray*}\label{MMinf.pi}
\pi_{0}=e^{\rho}\\
\pi_{n}=e^{-\rho}\frac{\rho^{n}}{n!}
\end{eqnarray*}
Entonces, el n?mero promedio de servidores ocupados es equivalente
a considerar el n?mero de clientes en el  sistema, es decir,
\begin{eqnarray*}
L=\esp\left[N\right]=\rho\\
Var\left[N\right]=\rho
\end{eqnarray*}
...
%
%%%\begin{frame}\frametitle{Cola $M/M/m$}
Este sistema considera $m$ servidores id?nticos, con tiempos entre
...
utilizaci?n por servidor como $u=\frac{\rho}{m}$ que tambi?n se
puede interpretar como la fracci?n de tiempo promedio que cada
servidor est? ocupado.\smallskip

La cola $M/M/m$ se puede considerar como un proceso de nacimiento
y muerte con par?metros: $\beta_{n}=\beta$ para $n=0,1,2,\ldots$ y
$\delta_{n}=\left\{\begin{array}{cc}
n\delta & n=0,1,\ldots,m-1\\
c\delta & n=m,\ldots\\
\end{array}\right.$

entonces  la condici?n de recurrencia se va a cumplir s? y s?lo si
...
%_____________________________________________________________________________________
%
%%%\begin{frame}\frametitle{Cadena de Markov con dos estados}
...
entonces despu?s de un procedimiento m?s o menos corto se tiene
que:
...
finito de $E$ tal que se cumple la ecuaci?n 5.2 para alguna
funci?n $h$ acotada que satisface
$h\left(i\right)<h\left(j\right)$ para alg?n estado $i\notin
...
visitados por $Y_{0},Y_{1},\ldots$, as? las trayectorias
muestrales son constantes entre $S_{n}$ consecutivos, continua por
la derecha, es decir, $X_{S_{n}}=Y_{n}$. \item La descripci?n de
un modelo pr?ctico est? dado usualmente en t?rminos de las
intensidades $\lambda\left(i\right)$ y las probabilidades de salto
$q_{ij}$ m?s que en t?rminos de la matriz de transici?n $P^{t}$.
\item Sup?ngase de ahora en adelante que $q_{ii}=0$ cuando
...
proceso de saltos $\left\{X_{t}\right\}_{t\geq0}$ est? dada por
\begin{eqnarray*}
\lambda\left(i,j\right)&=&\lambda\left(i\right)q_{i,j}\textrm{,    }j\neq i\\
\lambda\left(i,i\right)&=&-\lambda\left(i\right)\\
\end{eqnarray*}
...
\end{eqnarray*}
Adem?s, $\Lambda$ est? en correspondencia uno a uno con la
distribuci?n del proceso minimal dado por el teorema 3.1.
\end{Prop}
...
%%%\begin{frame}\frametitle{Matriz Intensidad}
Para el caso particular de la Cola $M/M/1$, la matr?z de itensidad
est? dada por
\begin{eqnarray*}
\Lambda=\left[\begin{array}{cccccc}
-\beta & \beta & 0 &0 &0& \cdots\\
...
%%%\begin{frame}\frametitle{Criterios de Ergodicidad}
\begin{Prop}
Si el proceso es erg?dico, entonces existe una versi?n
...
\begin{Teo}
Si $\left\{X_{t}\right\}$ es erg?dico y $\pi$ es la distribuci?n
...
erg?dica, es decir $|v|=\infty$, entonces $p_{ij}^{t}\rightarrow0$
para todo $i,j\in E$.
\end{Cor}

\begin{Cor}
Para cualquier proceso Markoviano de Saltos minimal, irreducible o
no, los l?mites $li_{t\rightarrow\infty}p_{ij}^{t}$ existe.
\end{Cor}
%\end{frame}
%_____________________________________________________________________________________
%
...
estados a lo m?s numerable, con la propiedad de que s?lo puede ir
...
$\delta_{n}$ las intensidades de muerte, o tambi?n se puede ver
como a $X_{t}$ el n?mero de usuarios en una cola al tiempo $t$, un
salto hacia arriba corresponde a la llegada de un nuevo usuario y
un salto hacia abajo como al abandono de un usuario despu?s de
...
adem?s se asumne por el momento que $p_{n}$ no puede tomar el
valor $0$ ? $1$ para cualquier valor de $n$.
%\end{frame}
...
tienen una distribuci?n general $G$. Sea
$N\left(t\right)_{t\geq0}$ el n?mero de clientes en el sistema al
tiempo $t$, y sean $t_{1}<t_{2}<\dots$ los tiempos sucesivos en
los que los clientes completan su servicio y salen del sistema.

La sucesi?n $\left\{X_{n}\right\}$ definida por
$X_{n}=N\left(t\right)$ es una cadena de Markov, en espec?fico es
la Cadena encajada del proceso de llegadas de usuarios. Sea
$U_{n}$ el n?mero de clientes que llegan al sistema durante el
tiempo de servicio del $n$-?simo cliente, entonces se tiene que
...
par?metro $\lambda$, la probabilidad condicional de que lleguen
...
\end{equation}
donde $G$ es la distribuci?n de los tiempos de servicio. Las
probabilidades de transici?n de la cadena est?n dadas por
\begin{equation}
...
distribuci?n estacionaria est? dada por
\begin{eqnarray}
\pi_{j}&=&\pi_{0}a_{j}+\sum_{i=1}^{j+1}\pi_{i}a_{j-i+1}\textrm{, para }j=0,1,\ldots\\
\pi_{0}&=&1-\rho
\end{eqnarray}
...
De las f?rmulas de Little, se tiene que
$W=E\left(w\right)=\frac{L}{\beta}$, tambi?n el tiempo de espera
en la cola
\begin{equation}
W_{q}=\esp\left(q\right)=\esp\left(w\right)-\esp\left(s\right)=\frac{L}{\beta}-\frac{1}{\delta},
\end{equation}
...
%_____________________________________________________________________________________
%
%%%\begin{frame}\frametitle{Cola M/M/m/m}
Consideremos un sistema con $m$ servidores id?nticos, pero ahora
...
determinan la distribuci?n estacionaria se tiene que
...
\end{equation}
A la ecuaci?n \ref{Eq.13.1} se le llama {\em distribuci?n
truncada}.

Si definimos
$\pi_{m}=B\left(m,\rho\right)=\pi_{0}\frac{\rho^{m}}{m!}$,
$\pi_{m}$ representa la probabilidad de que todos los servidores
se encuentren ocupados, y tambi?n se le conoce como {\em f?rmula
de p?rdida de Erlang}.

Necesariamente en este caso el tiempo de espera en la cola $W_{q}$
y el n?mero promedio de clientes en la cola $L_{q}$ deben de ser
cero puesto que no se permite esperar para recibir servicio, m?s
a?n el tiempode espera en el sistema y el tiempo de serivcio
tienen la misma distribuci?n, es decir,
\[W\left(t\right)=\prob\left\{w\leq t\right\}=1-e^{-\mu t}\], en
particular
\[W=\esp\left[w\right]=\esp\left[s\right]=\frac{1}{\delta}\]
%\end{frame}
%%%\begin{frame}\frametitle{Cola M/M/m/m}
Por otra parte, el n?mero esperado de clientes en el sistema es
\begin{eqnarray*}
...
%_____________________________________________________________________________________
%
\subsection{Colas en Serie}
%_____________________________________________________________________________________
%
%%%\begin{frame}\frametitle{Colas en Serie}

%\end{frame}
%_____________________________________________________________________________________
%
\subsection{Redes Abiertas de Jackson}
%_____________________________________________________________________________________
%
%%%\begin{frame}\frametitle{Redes Abiertas de Jackson}

%\end{frame}
%_____________________________________________________________________________________
%
\subsection{Colas C?clicas}
%_____________________________________________________________________________________
%
%%%\begin{frame}\frametitle{Colas C?clicas}
...
La sucesi?n $\left\{X_{n}\right\}$ definida por
$X_{n}=N\left(t_{n}\right)$ es una cadena de Markov, en espec?fico
...
%____________________________________________________________________________
%%%\begin{frame}\frametitle{Cola M/G/1}
Si la ecuaci?n \ref{Eq.19.6} la multiplicamos por $z^{j}$ y
...
A\left(1\right)=1$, y adem?s $A^{'}\left(z\right)\rightarrow
...
%_____________________________________________________________________________________
%
%%%\begin{frame}\frametitle{Sistemas Abiertos - 2 Servidores}
...
$\lambda_{1}$ al primer servidor, despu?s de ser atendido se pasa
a la siguiente cola en el segundo servidor. Cada servidor atiende
a un usuario a la vez con tiempo exponencial con raz?n $\mu_{i}$,
para $i=1,2$. A este tipo de sistemas se les conoce como siemas
secuenciales.

Def?nase el par $\left(n,m\right)$ como el n?mero de usuarios en
...
igual manera el proceso de salida de una cola $M/M/1$ con raz?n
$\lambda$, nos permite asumir que el servidor 2 tambi?n es una
cola $M/M/1$. Adem?s la probabilidad de que haya $n$ usuarios en
el servidor 1 es
\begin{eqnarray*}
...
\end{eqnarray*}
Si el n?mero de usuarios en los servidores 1 y 2 son variables
...
\end{eqnarray*}
entonces efectivamente la ecuaci?n (\ref{Eq.8.16}) satisface las
ecuaciones de balance (\ref{Eq.Balance}). El n?mero promedio  de
usuarios en el sistema, est? dado por
\begin{eqnarray*}
